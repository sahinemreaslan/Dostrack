...
class TrulyDynamicCenterPredictor(nn.Module):
    """
    Özellik haritası boyutunu (feat_sz) dinamik olarak alarak çalışan,
    tamamen dinamik bir CenterPredictor başlığı.
    """
    def __init__(self, inplanes=64, channel=256, stride=16, freeze_bn=False):
        """
        __init__ fonksiyonundan 'feat_sz' kaldırıldı. Artık bu değer sabit değil.
        """
        super(TrulyDynamicCenterPredictor, self).__init__()
        self.stride = stride

        # Mimarinin geri kalanı (conv katmanları) zaten dinamik olduğu için aynı kalır.
        # corner predict
        self.conv1_ctr = conv(inplanes, channel, freeze_bn=freeze_bn)
        self.conv2_ctr = conv(channel, channel // 2, freeze_bn=freeze_bn)
        self.conv3_ctr = conv(channel // 2, channel // 4, freeze_bn=freeze_bn)
    ...
    def forward(self, x, gt_score_map=None):
        """
        Forward pass. Gelen 'x' tensörünün boyutlarını burada dinamik olarak alır.
        """
        score_map_ctr, size_map, offset_map = self.get_score_map(x)

        # Bounding box hesaplaması için dinamik boyutları kullan
        if gt_score_map is None:
            bbox = self.cal_bbox(score_map_ctr, size_map, offset_map)
        else:
            bbox = self.cal_bbox(gt_score_map.unsqueeze(1), size_map, offset_map)
    ...
    def cal_bbox(self, score_map_ctr, size_map, offset_map, return_score=False):
        """
        Koordinat hesaplaması artık 'self.feat_sz' yerine gelen haritanın
        gerçek boyutlarına göre yapılır.
        """
        # Gelen skor haritasının gerçek uzamsal boyutlarını al (Yükseklik, Genişlik)
        feat_h, feat_w = score_map_ctr.shape[-2:]

        # En yüksek skoru ve onun düzleştirilmiş (flattened) dizideki indeksini bul
        max_score, idx = torch.max(score_map_ctr.flatten(1), dim=1, keepdim=True)
        
        # İndeksi, sabit 'feat_sz' yerine dinamik 'feat_w' kullanarak (x, y) koordinatlarına çevir
        idx_y = idx // feat_w
        idx_x = idx % feat_w

        # 'gather' fonksiyonu için indeksi hazırla
        idx = idx.unsqueeze(1).expand(idx.shape[0], 2, 1)
        size = size_map.flatten(2).gather(dim=2, index=idx)
        offset = offset_map.flatten(2).gather(dim=2, index=idx).squeeze(-1)

        # Bounding box'ı oluştururken normalizasyon için dinamik boyutları kullan
        bbox = torch.cat([(idx_x.to(torch.float) + offset[:, :1]) / feat_w,      # Genişliğe böl
                          (idx_y.to(torch.float) + offset[:, 1:]) / feat_h,      # Yüksekliğe böl
                          size.squeeze(-1)], dim=1)
    ...
    def get_score_map(self, x):
        """Bu fonksiyon zaten dinamik olduğu için bir değişiklik gerekmez."""
        def _sigmoid(x):
            y = torch.clamp(x.sigmoid_(), min=1e-4, max=1 - 1e-4)
            return y
...
def build_box_head(cfg, hidden_dim):
    ...
    if cfg.MODEL.HEAD.TYPE == "MLP":
        ...
        center_head = CenterPredictor(inplanes=in_channel, channel=out_channel,
                                      feat_sz=feat_sz, stride=stride)
        return center_head
    elif cfg.MODEL.HEAD.TYPE == "TrulyDynamicCenterPredictor":
        in_channel = hidden_dim
        out_channel = cfg.MODEL.HEAD.NUM_CHANNELS
        # TrulyDynamicCenterPredictor doesn't need feat_sz parameter
        center_head = TrulyDynamicCenterPredictor(inplanes=in_channel, channel=out_channel,
                                      stride=stride)
        return center_head
    else:
